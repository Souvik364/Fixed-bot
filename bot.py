...
async def type_animation(update: Update, context, text="💬 Bot is typing…", delay=1.5):
    """
    1. Sends the text '💬 Bot is typing…'
    2. Shows the header 'typing' status.
    3. Waits.
    4. Deletes the '💬 Bot is typing…' message.
    """
    msg = None
    try:
        # Send visible text message
...
async def send_temp_confirmation(update: Update, text="Message sent ✅", delay=5):
    """Sends a confirmation and deletes it after 'delay' seconds."""
    try:
        msg = await update.message.reply_text(text)
        await asyncio.sleep(delay)
        await msg.delete()
